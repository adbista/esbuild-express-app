...
exports.getNonEmptyEnvVar = getNonEmptyEnvVar;
exports.parseEnvBooleanString = parseEnvBooleanString;
exports.getEnvBoolean = getEnvBoolean;
exports.getEnvNumber = getEnvNumber;
exports.deduplicate = deduplicate;
exports.getEnvArray = getEnvArray;
exports.getEnvValueByPrecedence = getEnvValueByPrecedence;
exports.assertNoExtraneousProperties = assertNoExtraneousProperties;
exports.toDiagLogLevel = toDiagLogLevel;
exports.parseLogLevel = parseLogLevel;
exports.ensureResourcePath = ensureResourcePath;
...
function findServiceName(cache = configCache) {
    ...
    try {
        ...
        api_1.diag.debug(`Error reading ${pkgPath}`, e);
    }
    return undefined;
}
function defaultServiceName(cache = configCache) {
    return findServiceName(cache) || 'unnamed-node-service';
}
function getNonEmptyEnvVar(key) {
    const value = process.env[key];
    if (value !== undefined) {
        const trimmed = value.trim();
        if (trimmed.length === 0) {
            api_1.diag.warn(`Defined, but empty environment variable: '${key}'. The value will be considered as undefined.`);
            return undefined;
        }
        return trimmed;
    }
...
function parseEnvBooleanString(value) {
...
function getEnvValueByPrecedence(keys, defaultValue) {
...
const formatStringSet = (set) => {
    return [...set.values()].map((item) => `"${item}"`).join(', ');
};
function assertNoExtraneousProperties(
...
obj, expectedProps) {
    ...
    assert_1.strict.equal(keys.size, 0, `Unexpected configuration options: ${formatStringSet(keys)}. Allowed: ${formatStringSet(expectedProps)}`);
}
function validLogLevel(level) {
    return ['verbose', 'debug', 'info', 'warn', 'error'].includes(level);
}
function toDiagLogLevel(level) {
    switch (level) {
        case 'verbose':
...
function ensureResourcePath(url, resourcePath) {
    if (url === undefined) {
        return undefined;
    }
    const validUrl = constructUrl(url);
    if (validUrl === undefined) {
        api_1.diag.error(`Invalid URL: ${url}`);
        return undefined;
    }
    if (validUrl.pathname === '/') {
        api_1.diag.debug(`Appending path ${resourcePath} to ${url} due to resource path not set.`);
